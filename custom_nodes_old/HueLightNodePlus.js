class HueLightNodePlus extends LiteGraph.LGraphNode {
    ...
    onExecute() {
        ...
        if (hsvInput !== undefined) {
            ...
            if (hsvString !== JSON.stringify(this.properties.hsv)) {
                ...
                console.log('HSV input received, updating light state:', hsvInput);
                this.updateLightState(); // Call to update light state
            }
        }
    ...
    configure(data) {
        ...
        this.title = `Hue Light - ${this.properties.light_name}`;
        this.fetchAndAddLightDropdown(); // Refresh light dropdown
        this.updateColorSwatch(); // Update the color swatch on restore
    }
}

// Register the new HueLightNodePlus
