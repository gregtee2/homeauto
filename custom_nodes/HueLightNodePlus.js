class HueLightNodePlus extends LiteGraph.LGraphNode {
    ...
    fetchAndAddLightDropdown() {
        fetch(`http://${this.properties.bridge_ip}/api/${this.properties.api_key}/lights`)
    ...
    onExecute() {
        if (!this.properties.light_id) {
            return; // Exit if no light is selected
        }
    ...
    updateLightState() {
        ...
        fetch(`http://${this.properties.bridge_ip}/api/${this.properties.api_key}/lights/${this.properties.light_id}/state`, {
            method: 'PUT',
            headers: {
